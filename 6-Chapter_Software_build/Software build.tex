...
\begin{itemize}
    \item Private Build, the developer in its own private work space run the build process. As reported next, some build software allow for partial build. The developer can only "run" the part in which the changes have been applied. This is particularly useful since local build can be computationally heavy and time consuming for developer. 
    \item Integration build, after a private build the developer commit its changes to the current baseline branch. The sum of changes that the different developer implement are then run together, composing a integration build. 
...
\end{itemize}
\subsection{Software build systems}
