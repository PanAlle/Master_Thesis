...
\section{SQLAlchemy}
SQLALchemy is a Python package that creates an abstraction (\gls{ORM}) that connect the rows of a relational database to objext in aprogramming language. Via SQLAlchemy the programmer instead of writing SQL queries, uses Pyhton classes for the design of tables and functions to access/modify the tables. SQLAlchemy uses the \gls{ORM} layer to interface between python code and database instances. 
\gls{ORM} or Object Relational Mapping is a way to convert between rows in a relational database to an object in a programming language \cite{10.1145/1376616.1376773}. The \gls{ORM} sits between the programmer’s application and the database and helps the programmer by abstracting the SQL queries to access, remove and modify the data in the database.\\
SQLAlchemy can be connected to a wide variety of DBMS. To be able to connect the database there is the need of a driver. The function of the driver is the one of translating the SQL queries send by the application to the underlying format the database is using. The scheme of the connection is reported in the following block diagram. 
